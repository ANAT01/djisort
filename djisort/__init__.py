...
from djisort.modules.extract_exif_data import ImageMetaData
from djisort.modules.remove_empty_folders import removeEmptyFolders
...
@click.command()
@click.argument('src', type=click.Path(dir_okay=True), nargs=1)
@click.argument('dst', type=click.Path(dir_okay=True), nargs=1)
def djisort(src, dst):
    fpool = {}
